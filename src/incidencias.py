...
def listar_incidencias_activas(conn: MySQLConnection) -> list[dict]:
    ...
    """
    sql = (
        "SELECT * FROM incidencias WHERE estado <> 'cerrada' "
        "ORDER BY CASE prioridad WHEN 'alta' THEN 1 WHEN 'media' THEN 2 WHEN 'baja' THEN 3 ELSE 4 END, fecha_apertura ASC;"
    )
...
def listar_incidencias_sin_tecnico(conn: MySQLConnection) -> list[dict]:
    ...
    """
    sql = (
        "SELECT * FROM incidencias WHERE tecnico_id IS NULL AND estado <> 'cerrada' "
...
def crear_incidencia(conn: MySQLConnection, equipo_id: int, descripcion: str, prioridad: str = "media") -> int:
    ...
    sql = (
        "INSERT INTO incidencias (equipo_id, descripcion, prioridad, tecnico_id, estado, fecha_apertura, fecha_cierre) "
...
def asignar_tecnico(conn: MySQLConnection, incidencia_id: int, tecnico_id: int) -> int:
    ...
    sql = (
        "UPDATE incidencias SET tecnico_id = %s, estado = 'en_proceso' "
...
def detalle_incidencias_join(conn: MySQLConnection) -> list[dict]:
    ...
    """
    sql = (
        "SELECT i.id, i.descripcion, i.prioridad, i.estado, i.fecha_apertura, i.fecha_cierre, "
        "e.tipo, e.modelo, e.ubicacion, e.estado AS estado_equipo, t.nombre AS tecnico, i.tecnico_id "
        "FROM incidencias i "
        "JOIN equipos e ON i.equipo_id = e.id "
        "LEFT JOIN tecnicos t ON i.tecnico_id = t.id "
        "ORDER BY i.estado, CASE i.prioridad WHEN 'alta' THEN 1 WHEN 'media' THEN 2 WHEN 'baja' THEN 3 ELSE 4 END, i.fecha_apertura ASC;"
    )
    return fetch_all(conn, sql)
